...
// ── Step 1 integration: token logo resolution ─────────────────────────────
import { TokenLogo }        from '../../hooks/useTokenMeta';
import { resolveTokenMeta, toDbRecord } from '../../lib/tokenResolver';
...
function TradeContent() {
  ...
  const handleFetchToken=useCallback(async()=>{
    ...
    try{
      // Use tokenResolver — applies best-pair scoring + logo fallback chain
      const meta = await resolveTokenMeta(addr);
      if(meta.priceUsd===0 && !meta.pairAddress){
        // Resolved as fallback (DexScreener returned nothing)
        setFetchTokenError('Token not found on DexScreener. You can still add with fallback logo.');
        // Still show the fallback so user can confirm-add
        setFetchedTokenData({
          symbol:meta.symbol, name:meta.name, logoUrl:meta.logoUrl,
          basePrice:0, price:0, priceChange24h:0, volume24h:0,
      ...
      setFetchedTokenData({
        ...
        color:'from-cyan-400 to-blue-600', priceDirection:'neutral' as const,
        trending:false, coingeckoId:null, isCustom:true,
        mintAddress:addr, lpBurned:false,
        // Extra metadata saved to DB on addToken
        _pairAddress: meta.pairAddress,
        _dexId:       meta.dexId,
        _dexUrl:      meta.dexUrl,
        _liquidityUsd:meta.liquidityUsd,
      } as Token & { _pairAddress?:string|null; _dexId?:string|null; _dexUrl?:string|null; _liquidityUsd?:number });
    }catch(e){setFetchTokenError('Fetch failed. Try again.');}
  ...
  const handleAddToken=useCallback(async()=>{
    ...
    // Persist token metadata to DB so future battles resolve logo without re-fetching
    const addr = (fetchedTokenData as Token & {mintAddress?:string}).mintAddress;
    if(addr){
      const extra = fetchedTokenData as Token & {_pairAddress?:string|null;_dexId?:string|null;_dexUrl?:string|null;_liquidityUsd?:number};
      sbInsert('mr_token_metadata',{
        address:      addr,
        symbol:       fetchedTokenData.symbol,
        name:         fetchedTokenData.name,
        logo_url:     fetchedTokenData.logoUrl,
        pair_address: extra._pairAddress ?? null,
        dex_id:       extra._dexId       ?? null,
        dex_url:      extra._dexUrl       ?? null,
        price_usd:    fetchedTokenData.price,
        liquidity_usd:extra._liquidityUsd ?? 0,
        volume_24h:   fetchedTokenData.volume24h,
        change_24h:   fetchedTokenData.priceChange24h,
        source:       fetchedTokenData.price > 0 ? 'dexscreener' : 'fallback',
        updated_at:   new Date().toISOString(),
      }).catch(()=>{}); // non-fatal — cache only
    }
          ...
          <div className="px-5 pb-5 space-y-4">
            ...
            <div className="text-center pt-1">
              <div className="flex items-center justify-center gap-3 mb-2">
                <TokenLogo symbol={activeBattle.tokenA} logoUrl={gl(activeBattle.tokenA)} size={40} className="border-2 border-orange-500/40"/>
                ...
                <TokenLogo symbol={activeBattle.tokenB} logoUrl={gl(activeBattle.tokenB)} size={40} className="border-2 border-amber-500/40"/>
              </div>
                  ...
                  {!rushPosition?(
                          ...
                          return(
                            <div key={tok} className="rounded-2xl border border-white/[.06] overflow-hidden" style={{background:'rgba(18,12,4,.8)'}}>
                              <div className="flex items-center gap-2 p-3 border-b border-white/[.04]">
                                <TokenLogo symbol={tok} logoUrl={gl(tok)} size={28} className="border border-white/10"/>
                                <div>
                    ...
                    <>
                      ...
                      <div className="rounded-2xl border p-4 space-y-3" style={{background:'rgba(18,12,4,.9)',borderColor:rushPnL>=0?'rgba(74,222,128,.25)':'rgba(248,113,113,.25)'}}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <TokenLogo symbol={rushPosition.token} logoUrl={gl(rushPosition.token)} size={32} className="border border-white/10"/>
                            <div>
