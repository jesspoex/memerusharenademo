...
// =============================================================================
// BattleChartSection
// Self-contained — mounts only when chart tab open. Hook runs only then.
// =============================================================================
interface BattleChartSectionProps {
  tokenASymbol:    string;
  tokenBSymbol:    string;
...
const BattleChartSection = React.memo(function BattleChartSection({
  ...
  tokenALogoUrl, tokenBLogoUrl,
  tokenAPrice = 0, tokenBPrice = 0,
  tokenAChange24h = 0, tokenBChange24h = 0,
}: BattleChartSectionProps) {
  const { data, interval, activeToken, changeInterval, changeToken } = useBattleChart({
    tokenASymbol, tokenBSymbol,
    tokenAPrice, tokenBPrice,
    tokenAChange24h, tokenBChange24h,
  });

  const logoUrl = activeToken === 'A' ? tokenALogoUrl : tokenBLogoUrl;
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-orange-500/10" style={{background:'rgba(6,4,2,.99)'}}>
      ...
      <div className="px-1 pb-1" style={{minHeight:196}}>
        {data.loading ? (
          <div className="flex flex-col items-center justify-center gap-2.5 rounded-xl"
            style={{height:196,background:'rgba(249,115,22,.025)'}}>
            <div className="w-5 h-5 rounded-full border-2 animate-spin"
              style={{borderColor:'rgba(249,115,22,.4)',borderTopColor:'#f97316'}}/>
            <p className="text-[10px] text-slate-600">Loading chart…</p>
          </div>
        ) : data.error ? (
          <div className="flex flex-col items-center justify-center gap-1 rounded-xl"
            style={{height:196,background:'rgba(239,68,68,.025)'}}>
            <p className="text-[11px] text-red-400 font-bold">Chart unavailable</p>
            <p className="text-[10px] text-slate-600">{data.error}</p>
          </div>
        ) : data.candles.length === 0 ? (
          <div className="flex items-center justify-center rounded-xl"
            style={{height:196,background:'rgba(249,115,22,.02)'}}>
            <p className="text-[11px] text-slate-600">No chart data</p>
          </div>
        ) : (
          <TradeChart candles={data.candles} height={196} colorUp="#22c55e" colorDn="#f97316"/>
        )}
      </div>

      {data.pairAddress && (
        <div className="px-4 pb-2.5 flex justify-end">
          <a href={`https://dexscreener.com/solana/${data.pairAddress}`}
            target="_blank" rel="noopener noreferrer"
            className="text-[9px] text-slate-700 hover:text-orange-400 transition-colors font-mono tracking-wider">
            DEXSCREENER ↗
          </a>
        </div>
      )}
  ...
  const handleFetchToken=useCallback(async()=>{
    ...
    try{
      ...
      if(!meta.pairAddress&&meta.priceUsd===0){
        setFetchTokenError('Not found on DexScreener — fallback logo will be used.');
      }
      setFetchedTokenData({
            ...
            <div className="flex flex-wrap gap-2">
              {tokens.map(tok=>(
                <button key={tok.symbol} onClick={()=>{if(!createTokenA||createTokenA===tok.symbol)setCreateTokenA(tok.symbol);else if(createTokenB!==tok.symbol)setCreateTokenB(tok.symbol);}} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-xs font-bold transition-all ${createTokenA===tok.symbol||createTokenB===tok.symbol?'border-orange-500 text-white':'border-white/10 text-slate-300 hover:border-slate-500'}`} style={{background:createTokenA===tok.symbol||createTokenB===tok.symbol?'rgba(249,115,22,.2)':'rgba(30,41,59,.5)'}}>
                  <TokenLogo symbol={tok.symbol} logoUrl={tok.logoUrl||ph(tok.symbol)} size={16}/>{tok.symbol}
                </button>
              ))}
