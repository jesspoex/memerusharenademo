...
export function useBattleData(
  ...
  const loadWinners = useCallback(async () => {
    try {
      ...
      const data = await res.json() as {
        winners?: Array<{
          wallet: string; fullWallet: string; amountSol: number;
          battle: string; txHash?: string; time: string;
        }>;
      };
        ...
        setLeaderboard(
          Object.entries(grouped)
            ...
            .map(([w, d], i) => ({
              rank:     i + 1,
              wallet:   w,
              earnings: parseFloat(d.total.toFixed(3)),
              wins:     d.wins,
            })),
        );
      }
  ...
  // ── Auto-respawn ─────────────────────────────────────────────────────────────
  //
  // FIX: Call /api/ensure-battles (server-side, service_role, bypasses RLS).
  // The previous version used sbInsert directly with the anon key, which is
  // blocked by RLS → battles were never created → arena always empty.
  //
  // Use a ref for the latest version to avoid stale closures in the initial
  // useEffect which has deps=[].
  //
  const checkAndRespawnFn = useCallback(async () => {
    if (respawnLock.current) return;
    respawnLock.current = true;

    try {
      // ── Primary path: ask server to ensure battles (service_role bypasses RLS)
      let serverOk = false;
      try {
        ...
        // Server unreachable — fall through to client fallback
      }

      // Always reload display
      await loadBattles();
      if (serverOk) return;

      // ── Fallback path: direct Supabase insert (only if server unreachable)
      // Note: this will be blocked by RLS if anon policy does not allow INSERT.
      // It exists only as a best-effort path during cold starts / API downtime.
      const now  = new Date();
      ...
      await Promise.all(pairs.map((pair, i) => {
        ...
        const end = new Date(now.getTime() + dur * 1_000);
  ...
  // Store latest version in a ref so initial useEffect (deps=[]) can call it
  // without becoming stale
  const checkAndRespawnRef = useRef(checkAndRespawnFn);
  useEffect(() => { checkAndRespawnRef.current = checkAndRespawnFn; }, [checkAndRespawnFn]);

  // Stable wrapper — identity never changes, always calls latest version
  const checkAndRespawn = useCallback(() => checkAndRespawnRef.current(), []);
  ...
  useEffect(() => {
    Promise.all([loadBattles(), loadStats(), loadActivity(), loadWinners(), loadHistory()])
      .then(() => {
        setDbLoaded(true);
        // Use ref to call latest checkAndRespawn without stale closure
        setTimeout(() => checkAndRespawnRef.current(), 800);
  ...
  // ── Periodic respawn — stable checkAndRespawn so effect runs once ────────────
  useEffect(() => {
