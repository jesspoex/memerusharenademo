    ...
    <div className="flex items-start justify-between mb-1">
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-[11px] font-black text-slate-400 leading-none tracking-widest uppercase">
            {symbol}
          </p>
          ...
          {(vol > 0 || liq > 0) && (
            <div className="flex items-center gap-2 mt-0.5">
              {vol > 0 && (
                <span className="text-[9px] text-slate-600">Vol {fmtK(vol)}</span>
              )}
              {liq > 0 && (
                <span className="text-[9px] text-slate-600">Liq {fmtK(liq)}</span>
              )}
            </div>
          )}
        </div>
